...
configurations = [
                   {"name": "model8a", "lr":0.01, "momentum": 0.99}, 
                   {"name": "model8d", "lr":0.001, "momentum": 0.90},
                ]
...
for config in configurations:
    ...
    model.fit(features, labels, epochs=1000, batch_size=4000, validation_data=(test_features, test_labels), callbacks=[tensorboard_callback])

    print("Testing model.....")
    results = model.evaluate(test_features, test_labels, batch_size=4000)
    model.save(f'models/{model_name}.h5')

    sample_match = ['Ornn', 'Diana', 'Yasuo', 'Ashe', 'Amumu', 'Quinn', 'Graves', 'Yone', 'Caitlyn', 'Senna']
    sample_match2 = ['Quinn', 'Graves', 'Yone', 'Caitlyn', 'Senna', 'Ornn', 'Diana', 'Yasuo', 'Ashe', 'Amumu']
